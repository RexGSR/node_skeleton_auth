...
const removeDaysFromDate = (originDate, removeFactor, format = "YYYY-MM-DD") => dayjs(originDate).subtract(removeFactor, "day").format(format)
...
const humanReadable = (date) => new Intl.DateTimeFormat("en-US", {
    timeZone: "Asia/Kolkata",
    dateStyle: "medium",
    timeStyle: "short",
})
    .format(
        new Date(date) - new Date(18000000 + 1800000),
    )
    .toString()
    .replace(/([,]\s[1][2][:][0][0])\s\w+/g, "");

const checkInRange = (from, to, check) => dayjs(check).isBetween(from, dayjs(to));
